...
function theron_init() {
	...
	elseif ( has_action( 'theron_option_name' ) ) {
		do_action( 'theron_option_name' );
	}
	// If the developer hasn't explicitly set an option id, we'll use a default
	else {
		$default_themename = get_option( 'stylesheet' );
		$default_themename = preg_replace("/\W/", "_", strtolower($default_themename) );
		$default_themename = 'theron_' . $default_themename;
		if ( isset( $theron_settings['id'] ) ) {
			if ( $theron_settings['id'] == $default_themename ) {
				// All good, using default theme id
			} else {
				$theron_settings['id'] = $default_themename;
	...
	register_setting( 'optionsframework', $theron_settings['id'], 'theron_validate' );
	// Change the capability required to save the 'optionsframework' options group.
	add_filter( 'option_page_capability_optionsframework', 'theron_page_capability' );
}
 ...
 * Examples usage:
 *
 * add_filter( 'theron_menu', function($menu) {
...
function theron_menu_settings() {

	$menu = array(
		'page_title' => __( 'Theron Theme Options', 'optionsframework'),
		'menu_title' => __('Theron Theme Options', 'optionsframework'),
		'capability' => 'edit_theme_options',
		'menu_slug' => 'options-framework',
		'callback' => 'theron_page'
	);

	return apply_filters( 'theron_menu', $menu );
}
...
<?php if (isset($_GET['page']) && $_GET['page'] == 'options-framework') {?>
	...
	/* <![CDATA[ */
		...
		jQuery('.documentation-tab').live('click', function()  {
		jQuery('#optionsframework .documentation').load('<?php echo get_stylesheet_directory_uri(); ?>/admin/documentation.php', function(){
...
add_action( 'theron_after_validate', 'theron_save_options_notice' );
...
function theron_adminbar() {
	...
	$wp_admin_bar->add_menu( array(
			'parent' => 'appearance',
			'id' => 'of_theme_options',
			'title' => __( 'Theron Theme Options', 'options_framework_theme' ),
			'href' => admin_url( 'themes.php?page=options-framework' )
		));
}

/**
 * Wrapper for theron_options()
 *
...
function &_theron_options() {
	...
	if ( !$options ) {
		...
		if ( $optionsfile = locate_template( $location ) ) {
			...
			} else if ( function_exists( 'theron_options' ) ) {
