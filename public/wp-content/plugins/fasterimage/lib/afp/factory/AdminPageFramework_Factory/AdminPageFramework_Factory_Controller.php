 ...
 */
abstract class fasterImage_AdminPageFramework_Factory_Controller extends fasterImage_AdminPageFramework_Factory_View {
    ...
    public function setFieldErrors($aErrors) {
        $GLOBALS['afasterImage_AdminPageFramework']['aFieldErrors'] = $this->oUtil->getElement($GLOBALS, array('afasterImage_AdminPageFramework', 'aFieldErrors'), array());
        if (empty($GLOBALS['afasterImage_AdminPageFramework']['aFieldErrors'])) {
            add_action('shutdown', array($this, '_replyToSaveFieldErrors'));
        }
        $_sID = md5($this->oProp->sClassName);
        $GLOBALS['afasterImage_AdminPageFramework']['aFieldErrors'][$_sID] = isset($GLOBALS['afasterImage_AdminPageFramework']['aFieldErrors'][$_sID]) ? $this->oUtil->uniteArrays($GLOBALS['afasterImage_AdminPageFramework']['aFieldErrors'][$_sID], $aErrors) : $aErrors;
    }
    public function hasFieldError() {
        return isset($GLOBALS['afasterImage_AdminPageFramework']['aFieldErrors'][md5($this->oProp->sClassName) ]);
    }
    public function setSettingNotice($sMessage, $sType = 'error', $asAttributes = array(), $bOverride = true) {
        $GLOBALS['afasterImage_AdminPageFramework']['aNotices'] = $this->oUtil->getElement($GLOBALS, array('afasterImage_AdminPageFramework', 'aNotices'), array());
        if (empty($GLOBALS['afasterImage_AdminPageFramework']['aNotices'])) {
            add_action('shutdown', array($this, '_replyToSaveNotices'));
        }
        $_sID = md5(trim($sMessage));
        if ($bOverride || !isset($GLOBALS['afasterImage_AdminPageFramework']['aNotices'][$_sID])) {
            ...
            $GLOBALS['afasterImage_AdminPageFramework']['aNotices'][$_sID] = array('sMessage' => $sMessage, 'aAttributes' => $_aAttributes + array('class' => $sType, 'id' => $this->oProp->sClassName . '_' . $_sID,),);
        }
    }
    public function hasSettingNotice($sType = '') {
        $_aNotices = $this->oUtil->getElementAsArray($GLOBALS, array('afasterImage_AdminPageFramework', 'aNotices'), array());
        if (!$sType) {
