...
if ( is_admin() ) {
	...
	add_action( "admin_print_styles-$of_page", 'theron_mlu_css', 0 );

	add_action( "admin_print_scripts-$of_page", 'theron_mlu_js', 0 );	
...
if ( ! function_exists( 'theron_mlu_init' ) ) {

	function theron_mlu_init () {

		register_post_type( 'theron', array(
...
if ( ! function_exists( 'theron_mlu_css' ) ) {



	function theron_mlu_css () {
...
if ( ! function_exists( 'theron_mlu_js' ) ) {



	function theron_mlu_js () {
 ...
 * - theron_mlu_get_silentpost()

 */



if ( ! function_exists( 'theron_medialibrary_uploader' ) ) {



	function theron_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		$int = theron_mlu_get_silentpost( $id );
		...
		$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload', 'therontheme' ) . '" rel="' . $int . '" />' . "\n";
 ...
 * If no "silent" post is present, one will be created with the type "theron"
 ...
 * theron_mlu_get_silentpost ( 'of_logo' );

 */



if ( ! function_exists( 'theron_mlu_get_silentpost' ) ) {



	function theron_mlu_get_silentpost ( $_token ) {
		...
		if ( $_token ) {
			...
			$_args = array( 'post_type' => 'theron', 'post_name' => 'of-' . $_token, 'post_status' => 'draft', 'comment_status' => 'closed', 'ping_status' => 'closed' );
...
if ( ! function_exists( 'theron_mlu_insidepopup' ) ) {



	function theron_mlu_insidepopup () {

	

		if ( isset( $_REQUEST['is_theron'] ) && $_REQUEST['is_theron'] == 'yes' ) {

		

			add_action( 'admin_head', 'theron_mlu_js_popup' );

			add_filter( 'media_upload_tabs', 'theron_mlu_modify_tabs' );
...
if ( ! function_exists( 'theron_mlu_js_popup' ) ) {



	function theron_mlu_js_popup () {
...
?>
	...
	jQuery(function($) {
		...
		// Preserve the "is_theron" parameter on the "delete" confirmation button.
		...
		$( '.savesend a.del-link' ).click ( function () {
			...
			continueHref = continueHref + '&is_theron=yes';
...
if ( ! function_exists( 'theron_mlu_modify_tabs' ) ) {



	function theron_mlu_modify_tabs ( $tabs ) {

		$tabs['gallery'] = str_replace( __( 'Gallery', 'theron' ), __( 'Previously Uploaded', 'theron' ), $tabs['gallery'] );
